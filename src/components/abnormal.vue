...
export default {
  ...
  methods:{
    //活动用电异常中的数据
		getAbnormalMsg(){
			this.$http.get(store.state.baseUrl+"/electricityAlarm/getElecAlarmList/"+store.state.roomId)
			.then(res=>{
			...
			for(var i=0;i<this.lists.length;i++){
				...
				//获得时间
				this.lists[i].alarmTime = store.state.utils.formatTime(this.lists[i].alarmTime);
