...
export default {
  ...
  methods:{
    getYearData(){
      //获得全年用电数据
      this.$http.get(store.state.baseUrl+"/electricQuery/queryByRoomId/"+store.state.roomId)
      .then((res)=>{
    ...
    getMonthData(){
      //获得月用电数据
      this.$http.get(store.state.baseUrl+"/electricQuery/queryByRoomIdByDay/41500521/"+this.selectYear+"/"+this.selectMonth)
      .then((res)=>{
        // console.log(res.data)
